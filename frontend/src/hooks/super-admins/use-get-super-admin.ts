...
export const useGetSuperAdmin = (id: number) => {
    const hasFetchedRef = useRef(false);

    const [data, setData] = useState<SuperAdminInterface | null>(null);
    ...
    const fetchHandler = async () => {
        try {
            ...
            const result = await superAdminsService.getByIdHandler(id);
